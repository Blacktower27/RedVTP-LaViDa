...
class LlavaLladaForMaskedDiffusion(LLaDAModelLM,LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels,new_input_ids, all_image_positions) = self.prepare_inputs_labels_for_multimodal(input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities, image_sizes,return_inputs=True)
        #prompt_lengths = 
        #breakpoint()
        # hack starts here
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _, all_image_positions, image_position, num_visual_tokens) = self.prepare_inputs_labels_for_multimodal(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes)
        else:
            # breakpoint()
        ...
        return llada_generate(self.get_model(),inputs_embeds=inputs_embeds,position_ids=position_ids,attention_mask=attention_mask,all_image_positions=all_image_positions, image_position=image_position, num_visual_tokens=num_visual_tokens,**kwargs)
    ...
    def log_likelyhood_inference(
        ...
        **kwargs,
        ...
        if "inputs_embeds" in kwargs:
            raise NotImplementedError("`inputs_embeds` is not supported")
        #change! return image_position and num_visual_tokens
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _, image_position, num_visual_tokens) = self.prepare_inputs_labels_for_multimodal(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes)
        else:
            inputs_embeds = self.get_model().embed_tokens(inputs)
        max_seq_len = 5000
