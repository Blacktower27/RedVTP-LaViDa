...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        output_attentions = False,#change!
    ) -> torch.Tensor:
        """
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            if output_attentions:#change!
                return self.scaled_dot_product_attention_with_map(
                    q,
                    k,
                    v,
                    attn_mask=None,
    ...
    #chage!
    def scaled_dot_product_attention_with_map(self, q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, training=False):
        B, nH, T_q, d_head = q.shape
        T_k = k.size(2)
        if scale is None:
    ...
    def attention(
        ...
        output_attentions: Optional[bool] = None,  # changed! add output_attentions!
        ...
        if block_mask is not None:
            ...
            if output_attentions:
                att, attn_weights = self._scaled_dot_product_attention(
                    ...
                    dropout_p=0.0 if not self.training else self.config.attention_dropout,
                    is_causal=False,
                    output_attentions = output_attentions#chage!
                )
            else:
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            if output_attentions :
                att, cache, attn_weight = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache,block_mask=block_mask, output_attentions = output_attentions)
            else: 
                att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache,block_mask=block_mask)
...
class LLaDAOutput(NamedTuple):
    ...
    """
    change!
    Attention probability maps from each attention block.
    Each tensor usually has shape `(batch, num_heads, seq_len, seq_len)`.
    """
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        #changed! create all_attn_weights to collect attention map in each layer
        ...
        #chage to return attention map
        return LLaDAOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None,  attentions=tuple(all_attn_weights) if output_attentions else None)  # type: ignore[arg-type]
